...
iris_image *iris_multiref_with_embeddings(
    ...
    /* No references - text-to-image */
    if (!refs || num_refs == 0) {
        return iris_generate_with_embeddings(ctx, text_emb, text_seq, params);
    }

    /* Single reference - use optimized path */
    if (num_refs == 1) {
        return iris_img2img_with_embeddings(
            ctx, text_emb, text_seq, refs[0], params
        );
    }

    if (!ctx->is_distilled) {
        fprintf(stderr, "Warning: iris_multiref_with_embeddings() does not "
                        "support CFG. Use iris_multiref() for base models.\n");
    }
    ...
    for (int i = 0; i < num_refs; i++) {
        ...
        if (!tensor) {
            ...
            free(resized_imgs);
            free(ref_pixel_dims);
            set_error("Failed to convert reference image to tensor");
            return NULL;
        }

        int lat_h, lat_w;
        ref_data[i] = iris_vae_encode(
            ctx->vae, tensor, 1, img_to_use->height, img_to_use->width, &lat_h, &lat_w
        );
        free(tensor);
    ...
    float *latent = iris_sample_euler_multirefs_flux(
        ctx->transformer, ctx->qwen3_encoder,
        z, 1, IRIS_LATENT_CHANNELS, latent_h, latent_w,
        ref_latents, num_refs,
        text_emb, text_seq,
